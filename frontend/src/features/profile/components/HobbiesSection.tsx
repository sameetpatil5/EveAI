...
export default function HobbiesSection({ hobbies }: { hobbies: Profile['hobbies'] }) {
  ...
  const displayedHobbies = localHobbies

  return (
    <div className="rounded-[10px] border border-[#e9eaf2] bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      <div className="mb-3 flex items-center justify-between">
        <div>
          <div className="text-xs font-semibold uppercase tracking-[0.18em] text-[#64748b]">Hobbies</div>
        </div>
        <Button
          variant="secondary"
      ...
      <div className="flex flex-wrap gap-2">
        {displayedHobbies.length > 0 ? (
          ...
          <div className="text-sm text-[#64748b]">[No hobbies added yet.]</div>
        )}
      </div>
