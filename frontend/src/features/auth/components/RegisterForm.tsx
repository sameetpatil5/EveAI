...
export function RegisterForm() {
  ...
  return (
    <div className="mx-auto w-full max-w-md rounded-3xl border border-[#e9eaf2] bg-white px-8 py-10 shadow-sm">
      <h1 className="mb-6 text-2xl font-semibold text-[#0f172a]">Create your EveAI account</h1>
      <form className="space-y-5" onSubmit={handleSubmit}>
        ...
        <Button type="submit" className="w-full" disabled={mutation.status === 'pending'}>
          {mutation.status === 'pending' ? 'Creating account...' : 'Register'}
        </Button>
      </form>
      <p className="mt-6 text-sm text-[#64748b]">
        Already have an account?{' '}
        <a href="/auth/login" className="font-medium text-[#607afb] hover:text-[#4f63df]">
          Login
        </a>
      </p>
    </div>
