    ...
    return (
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button className="rounded-2xl bg-[#607afb] px-5 py-2 text-white shadow-sm hover:bg-[#4f6ae8] transition" onClick={() => setOpen(true)}>
            Regenerate
          </button>
        </div>
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-[#64748b]">Your weekly routine is generated from your subjects, availability, and hobbies.</p>
          </div>
          <button className="rounded-2xl bg-white border border-[#607afb] px-6 py-2 font-semibold text-[#607afb] shadow-sm hover:bg-[#f5f7ff] transition" onClick={() => setOpen(true)}>
            Regenerate
          </button>
        </div>
        ...
        <div className="mb-6 flex gap-2 overflow-x-auto pb-2">
          {dayShort.map((label, idx) => (
            <button
              key={label}
              type="button"
              className={`min-w-[70px] rounded-2xl px-4 py-3 text-sm font-semibold transition ${selectedDayIdx === idx ? 'bg-[#607afb] text-white shadow-lg shadow-blue-100' : 'bg-white text-[#64748b] hover:text-[#607afb]'}`}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#f8f9fc]">
                  ...
                  <th className="sticky top-0 z-20 px-6 py-4 text-sm font-semibold uppercase text-[#64748b] bg-[#f8f9fc]">Status</th>
                </tr>
              </thead>
              <tbody>
